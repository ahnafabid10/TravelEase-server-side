...
const uri = "mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.mrbmxyw.mongodb.net/?appName=Cluster0";
...
res.send(result)
    ...
    app.get('/myVehiclePage', async(req,res)=>{
      const email = req.query.email
      const result = await productCollection.find({currentUserEmail : email}).toArray()
res.send(result)
    })
no
